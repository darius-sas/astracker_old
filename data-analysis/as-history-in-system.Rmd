...
if(is.null(environment()$project)){
  ...
  type <- svDialogs::dlgForm(list("type:CB"=c("consecOnly", "nonConsec")), title="Consecutive or non consecutive smells?")$res$type
}

similarity_scores_file <- paste("similarity-scores-", type, ".csv", sep = "")
similarity_scores_file <- paste("../test-data/output/trackASOutput", project, similarity_scores_file, sep="/")
characteristics_file <- paste("smell-characteristics-", type, ".csv", sep = "")
characteristics_file <- paste("../test-data/output/trackASOutput", project, characteristics_file, sep="/")
...
p2 <- ggplot(df, aes(version, group = affectedComponentType, fill = affectedComponentType)) + geom_bar()
grid.arrange(p, p2)
```
