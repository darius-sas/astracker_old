...
Select an input file and click the "Start analysis" button. Next, after selecting the characteristic which signal to analyse, click "Compute" and wait for the computation to complete and the plots to appear.
...
data <- eventReactive(input$calculate, {shinyjs::disable("calculate"); read.csv(input$dataset$datapath)})
...
renderPlot({
  ...
  ggplot(df, aes(versionPosition, !!sym(charact), group = uniqueSmellID, color = smellType)) + 
    ...
    labs(x = "Version Number",
         y = charact,
         title = "Characteristic evolution for each smell and for each project")
}, height = 900)
```
