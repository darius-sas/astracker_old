...
//		SortedMap<String, Graph> versionedSystem = ArcanDependencyGraphParser.parseGraphML(dir);
		...
		PersistenceWriter
				.register(new SmellSimilarityDataGenerator("test-data/jaccard-scores-"+name+"-consecutives-only.csv"));
		 PersistenceWriter.register(new
		 SmellCharacteristicsGenerator("test-data/"+name+"-smells-characteristics.csv", pr));
		// // this test is out of date, added null to allow compilation

		pr.forEach(v -> {
			logger.info("Tracking version {}- {}", v, v.getGraph());
			List<ArchitecturalSmell> smells = pr.getArchitecturalSmellsIn(v);
